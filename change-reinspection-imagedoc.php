...
<?php
	...
	{
		$id = $_GET['edit_id'];
		$stmt_edit = $DB_con->prepare('SELECT Desc1, Image FROM reinspectionimagesdoc WHERE id =:uid');
	...
	{
		header("Location: change-reinspection-imagedoc.php");
	}
	...
	if(isset($_POST['update']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE reinspectionimagesdoc
									     SET Desc1=:uname,										 
										     Image=:upic 
								       WHERE id=:uid');
...
$sql ="SELECT * from reinspectionimagesdoc where reinspectionimagesdoc.id=:id Limit 1";
...
<div class="col-sm-8">
<img src="reinspectionimages/<?php echo htmlentities($result->Image);?>" width="300" height="250" style="border:solid 1px #000">
</div>
