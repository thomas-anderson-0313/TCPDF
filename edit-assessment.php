...
$sql="UPDATE assessments set CostTotal=:itemstotal,DiscountDetails=:discountdetails,Discount=:discount,Discountt=:discountt,Net=:netparts where id=:id";
...
$sql="UPDATE costofrepairs set Number=:number,Quantity=:quantity,Description=:description,UnitCost=:unitcost,Total=:total where id=:id"; 
$query = $dbh->prepare($sql);
...
$sql="UPDATE assessments set SubTotal=:subtotal,Vat=:vat,GrandTotal=:grandtotal where id=:id";
$query = $dbh->prepare($sql);
...
$sql="INSERT INTO costofrepairs (Number,Quantity,Description,UnitCost,Total,ReportId) 
...
$sql="INSERT INTO costofrepairssummary (DescriptionSummary,TotalSummary,ReportIdSummary) 
...
$sql="UPDATE assessments set SubTotal=:subtotal,Vat=:vat,PartsVatDesc=:partsvatdesc,GrandTotal=:grandtotal where id=:id";
$query = $dbh->prepare($sql);
$query->bindParam(':subtotal',$subtotal,PDO::PARAM_STR);
$query->bindParam(':vat',$vat,PDO::PARAM_STR);
...
?>			  
			  ...
			  <a style="padding:10px;" class="btn btn-danger" href="assessments-admin"><<-Back</a>
			  
<?php } else{ ?> 			  
 
             <a style="padding:10px;" class="btn btn-danger" href="assessments"><<-Back</a>
			  
<?php } ?>	
		  
            </div>
          </div><hr>
		  <h5 style="color:green; padding:10px;"><strong>Update Motor Assessment Report</strong></h5>		  
...
<?php 
$x=intval($_GET['id']);
$sql ="SELECT Number FROM costofrepairs WHERE ReportId=$x ORDER BY id DESC LIMIT 1";
$query= $dbh -> prepare($sql);
$query-> execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
if($query->rowCount() > 0)
...
$namba3=$namba1+$namba2;
          ...
          <div class="form-group">
            <div class="form-row">
			  <div class="col-md-1">
                <div class="form-label-group">
                  <input type="text" id="number" name="number" value="<?php echo $namba3; ?>" class="form-control" placeholder="First name"  autofocus="autofocus">
			  ...
			  <div class="col-md-1">
                <div class="form-label-group">
                  <input type="text" id="contribution" name="contribution" oninput="calculate2();" value="0" class="form-control" placeholder="Last name" >
                  <label for="contribution">Con.(%)</label>
                </div>
              </div>
			  <div class="col-md-2">
                <div class="form-label-group">
                  <input type="text" id="unitcostt" name="unitcostt" class="form-control" oninput="calculate2();" placeholder="First name"  autofocus="autofocus">
                  <label for="unitcostt">Unit Cost</label>
                </div>
              </div>
                ...
                <div class="form-label-group">
			  	...
			  	<div class="col-md-2">
                <div class="form-label-group">
                  <input type="text" id="total" name="total" class="form-control" oninput="calculate2();" placeholder="First name" autofocus="autofocus" readonly>
		    ...
		    <div class="form-group">
			  ...
			  <div class="col-md-5"><br>
                <div class="form-label-group">
                  ...
                  <label for="discountt">Discount Amount - Example: for 7.5% discount enter 0.075</label>
                </div>
              </div>
...
{	?>								
			...
			<div class="col-md-3"><br>
                <div class="form-label-group">
                  <input type="text" id="partsvatdesc" name="partsvatdesc" class="form-control" placeholder="Last name" value="16% VAT" required>
...
<script>
    function calculate2() {
		...
		var myBox3 = document.getElementById('unitcostt').value;
		var myBox4 = document.getElementById('contribution').value;
        var results = document.getElementById('total'); 
		var myResult = Number(myBox1) * Number(myBox2);
          document.getElementById('total').value = myResult;
		var myResult = Number(myBox3) - Number(myBox4) / 100 * Number(myBox3);
		  document.getElementById('unitcost').value = myResult;
