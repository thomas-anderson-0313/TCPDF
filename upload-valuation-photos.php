...
$id=intval($_GET['id']);
move_uploaded_file($_FILES["img5"]["tmp_name"],"valuationimages/".$_FILES["img5"]["name"]);
$sql="update valuations set Image5=:image5 where id=:id";
...
$id=intval($_GET['id']);
move_uploaded_file($_FILES["img4"]["tmp_name"],"valuationimages/".$_FILES["img4"]["name"]);
$sql="update valuations set Image4=:image4 where id=:id";
$query = $dbh->prepare($sql);
...
<?php
	...
	if(isset($_POST['deletr']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT Image4 FROM valuations WHERE id =:delid');
		$stmt_select->execute(array(':delid'=>$_POST['deletr']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("valuationimages/".$imgRow['Image4']);
...
<?php
	...
	if(isset($_POST['updatephotos']))
	{
		...
		{
			$upload_dir = 'valuationimages/'; // upload directory
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO valuationimages(Desc1,Desc2,ReportId,Image1,Image2) VALUES(:desc1,:desc2,:reportid,:upic1,:upic2)');
...
move_uploaded_file($_FILES["img1"]["tmp_name"],"valuationimages/".$_FILES["img1"]["name"]);
$sql="INSERT INTO valuationimagesdoc(Image,ReportId) VALUES(:image,:reportid)";
$query = $dbh->prepare($sql);
...
<?php
	...
	if(isset($_POST['del']))
	{
		$delid=$_POST['id'];
		// select image from db to delete
		$stmt_select = $DB_con->prepare('SELECT Image1,Image2 FROM valuationimages WHERE id =:delid');
		$stmt_select->execute(array(':delid'=>$_POST['del']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("valuationimages/".$imgRow['Image1']);
		unlink("valuationimages/".$imgRow['Image2']);
		
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM valuationimages WHERE id =:delid');
...
<?php
	...
	if(isset($_POST['dell']))
	{
		$delid=$_POST['id'];
		// select image from db to delete
		$stmt_select = $DB_con->prepare('SELECT Image FROM valuationimagesdoc WHERE id =:delid');
		$stmt_select->execute(array(':delid'=>$_POST['dell']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("valuationimages/".$imgRow['Image']);
		
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM valuationimagesdoc WHERE id =:delid');
...
{	?>
<h4><b style="color:green; padding-left:30px;">Upload Valuation Photos (<?php echo htmlentities($result->RegNo);?>)</b></h4>
...
?>			  
			 ...
			 <a style="margin-left:800px;" class="btn btn-danger" href="valuations-admin"><<-Back</a>
			  
<?php } else{ ?> 			  
 
             <a style="margin-left:800px;" class="btn btn-danger" href="valuations"><<-Back</a>
      ...
      <div class="card mt-3 tab-card">
        <div class="card-header tab-card-header">
          <ul class="nav nav-tabs card-header-tabs" id="myTab" role="tablist">
			...
			<li class="nav-item">
                <a class="nav-link" id="three-tab" data-toggle="tab" href="#three" role="tab" aria-controls="Three" aria-selected="false">Fee Note ETR</a>
            </li>
          </ul>
        </div>
...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT * from valuationimages where ReportId=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
...
<div class="row">
<div class="col-sm-3"><img style="padding:3px;" src="valuationimages/<?php echo htmlentities($result->Image1);?>" class="img-responsive" width="110" height="90" alt="image"><br>
<p style="font-size:12px;"><?php echo htmlentities($result->Desc1);?></p>
<a style="font-size:12px;" href="change-valuation-image1?edit_id=<?php echo htmlentities($result->id)?>">Change image</a></div>
<div class="col-sm-3"><img style="padding:3px;" src="valuationimages/<?php echo htmlentities($result->Image2);?>" class="img-responsive" width="110" height="90" alt="image"><input hidden name="id" value="<?php echo htmlentities($result->id);?>"><br>
<p style="font-size:12px;"><?php echo htmlentities($result->Desc2);?></p>
<a style="font-size:12px;" href="change-valuation-image2?edit_id=<?php echo htmlentities($result->id)?>">Change image</a></div>
<button style="height:29px; padding:3px; margin-left:20px;" class="btn btn-danger" type="submit" name="del" value="<?php echo htmlentities($result->id)?>" ><i class="fa fa-trash"> Delete</i></button>
</form>
</div><br>
<?php }} ?>
</div>	</div>										
...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT * from valuationimagesdoc where ReportId=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
...
<form method="post" onSubmit="return valid();">
<div class="row">
<div><img style="padding:6px;" src="valuationimages/<?php echo htmlentities($result->Image);?>" class="img-responsive" width="110" height="150" alt="image"><input hidden name="id" value="<?php echo htmlentities($result->id);?>"><br>
<a style="font-size:12px;" href="change-valuation-imagedoc?edit_id=<?php echo htmlentities($result->id)?>">Change image</a></div><button style="height:29px; padding:3px;" class="btn btn-danger" type="submit" name="dell" value="<?php echo htmlentities($result->id)?>" ><i class="fa fa-trash"> Delete</i></button>
</div>
</form>
<?php }} ?>
</div>
...
<img style="padding:3px;" src="valuationimages/<?php echo htmlentities($result->Image4);?>" class="img-responsive" width="150" height="200" alt="No ETR Image">
