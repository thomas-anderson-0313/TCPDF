...
<?php
	...
	{
		$id = $_GET['edit_id'];
		$stmt_edit = $DB_con->prepare('SELECT Desc1, Image FROM valuationimagesdoc WHERE id =:uid');
	...
	{
		header("Location: change-valuation-imagedoc.php");
	}
	...
	if(isset($_POST['update']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE valuationimagesdoc
									     SET Desc1=:uname,										 
										     Image=:upic 
								       WHERE id=:uid');
...
$sql ="SELECT * from valuationimagesdoc where valuationimagesdoc.id=:id Limit 1";
...
<div class="col-sm-8">
<img src="valuationimages/<?php echo htmlentities($result->Image);?>" width="300" height="250" style="border:solid 1px #000">
</div>
