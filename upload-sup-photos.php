...
<?php
	...
	if(isset($_POST['updatedoc']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO supplementaryimagesdoc(ReportId,Image) VALUES(:reportid,:upic)');
...
<?php
	...
	if(isset($_POST['dell']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT Image FROM supplementaryimagesdoc WHERE id =:delid');
		$stmt_select->execute(array(':delid'=>$_POST['dell']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("supplementaryimages/".$imgRow['Image']);
		
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM supplementaryimagesdoc WHERE id =:delid');
...
{	?>
<h4><b style="color:green; padding-left:30px;">Upload Supplementary Photos (<?php echo htmlentities($result->RegistrationNo);?>)</b></h4>
<a style="margin-left:800px;" class="btn btn-danger" href="edit-supplementary-details?id=<?php echo htmlentities($result->id);?>"><< Back</a>
<?php }} ?>
...
<div class="row">
<div class="col-sm-3"><img style="padding:3px;" src="supplementaryimages/<?php echo htmlentities($result->Image1);?>" class="img-responsive" width="110px" height="90px" alt="image"><br>
<p style="font-size:12px;"><?php echo htmlentities($result->Desc1);?></p></div>
<div class="col-sm-3"><img style="padding:3px;" src="supplementaryimages/<?php echo htmlentities($result->Image2);?>" class="img-responsive" width="110px" height="90px" alt="image"><input hidden name="id" value="<?php echo htmlentities($result->id);?>"><br>
<p style="font-size:12px;"><?php echo htmlentities($result->Desc2);?></p></div>
...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT * from supplementaryimagesdoc where ReportId=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
...
<form method="post" onSubmit="return valid();">
<div class="row">
<div><img style="padding:6px;" src="supplementaryimages/<?php echo htmlentities($result->Image);?>" class="img-responsive" width="110" height="150" alt="image"><input hidden name="id" value="<?php echo htmlentities($result->id);?>"><br></div>
