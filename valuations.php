								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" height="2%">
									<thead style="color:green;">
										<tr>
											...
											<th style="width:30px; font-size:14px;">Insurance Ref</th>
									...
									<?php $sql = "SELECT * from valuations ORDER BY id desc";
$query = $dbh -> prepare($sql);
$query->execute();
...
<?php if($Astatus==0){
?>											
											<td style="font-size:14px;"><a href='upload-valuation-photos?id=<?php echo htmlentities($result->id);?>'><i class="fa fa-plus"> </i>Add Photos</a></td>
<?php } else{ ?> 
...
{
?><a class="btn btn-success btn-sm" href="valuations?aeid=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to Lock report for edit?')"> Mark Complete</a>
<?php } else {?>
