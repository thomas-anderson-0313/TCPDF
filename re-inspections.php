...
$eid=intval($_GET['eid']);
$status="0";
$sql = "UPDATE reinspections SET Status=:status WHERE  id=:eid";
...
$sql = "UPDATE reinspections SET Status=:status WHERE  id=:aeid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
    ...
    <div id="content-wrapper"><br><br>
      ...
      <div class="container-fluid">
        ...
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Re-inspection Reports</li>
        </ol>
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" height="2%">
									<thead style="color:green;">
										<tr>
											...
											<th style="width:110px; font-size:14px;">Insured</th>
											<th style="width:50px; font-size:14px;">Reg NO.</th>
											<th style="width:30px; font-size:14px;">Insurance Co.</th>
											<th style="width:60px; font-size:14px;">Preview</th>
									...
									<?php $sql = "SELECT Insured,RegistrationNo,Customer,Status,id from reinspections ORDER BY id desc";
$query = $dbh -> prepare($sql);
$query->execute();
...
{				?>	
										<tr>
											...
											<td><a href="preview-reinspection?id=<?php echo htmlentities($result->id);?>" ><i class="fa fa-eye"> Preview</i></a></td>
											
<?php										
$Astatus=($result->Status);?>
<?php if($Astatus==0){
?>												
											<td style="font-size:14px;"><a href='upload-reinspection-photos?id=<?php echo htmlentities($result->id);?>'><i class="fa fa-plus"> </i>Add Photos</a></td>
<?php } else{ ?> 
<td style="font-size:14px;"><i style="color:gray;" class="fa fa-plus"> Add Photos </i></td>
<?php } ?>											
...
<?php if($Astatus==0){
?>												
										    <td style="font-size:14px;"><a href='edit-reinspection-details?id=<?php echo htmlentities($result->id);?>'><i class="fa fa-edit"> Edit </i></a></td>
											
<?php } else{ ?> 
<td style="font-size:14px;"><i style="color:gray;" class="fa fa-edit"> Edit </i></td>
<?php } ?>											
...
{
?><a class="btn btn-success btn-sm" href="re-inspections?aeid=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to Lock report for edit?')"> Mark Complete</a>
<?php } else {?>

<a class="btn btn-danger btn-sm"> <i style="color:white;" class="fa fa-lock"> Locked</i></a>
</td>
<?php } ?>
