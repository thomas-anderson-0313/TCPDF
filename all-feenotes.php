    ...
    <div id="content-wrapper"><br><br>
      ...
      <div class="container-fluid">
        ...
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">All Fee Notes</li>
        </ol>
<?php 
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" height="3%">
									<thead style="color:green;">
										<tr>
											...
											<th style="width:130px;">Vehicle Make</th>
									...
									<?php $sql = "SELECT * from Assessments ORDER BY id desc";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
{				?>	
										<tr>
											...
											<td><a  href=""><?php echo htmlentities($result->Make);?></td>
                                            ...
                                            $vstatus=($result->PStatus);?>
											<?php if($vstatus==1){
											 echo "Paid";} elseif($vstatus==0) { echo "Not paid"; }else{ echo "Pending Payment";
											}?></strong></td>
											<td><?php if($result->PStatus=="" || $result->PStatus==0)
{
?><a class="btn btn-success btn-sm" href="all-feenotes?aeid=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to Mark Paid?')"> Mark Paid</a>
<?php } else {?>

<a class="btn btn-danger btn-sm" href="all-feenotes?eid=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to Mark Not Paid?')"> Mark Not Paid</a>
</td>
<?php } ?>
